...
def canfill(coordinate,m,n,a,b):
    '判断在一系列坐标下a*b的长方形能不能填充，并且返回方案'
    boole_1=False
    boole_2=False
    parts=[]
    if a!=b:
        for i in coordinate:
            list_1=seriescoordinate(i,a,b)[0]
            list_2=seriescoordinate(i,a,b)[1]
...
def issame(solution_1,solution_2):
    '判断两个方案是不是一样的'
    for i in solution_1:
        if i not in solution_2:
            return False
    return True



def isinsolution(solution_1,solution):
    '判断这个方案是不是已经在某个方案列表中'
    for i in solution:
        if issame(i,solution_1):
            return True
    return False



def repeatergetaway(solutions):
    '去掉方案中的重复项'
    solutions_nonrepeat=[]
    for i in solutions:
        if not isinsolution(i,solutions_nonrepeat):
            solutions_nonrepeat.append(i)
    return (solutions_nonrepeat)
...
def drawasquare(bob,left_corner,right_corner):
...
def turtles_see_you_again(solution,m,n,a,b):
    '小乌龟我们又见面了TAT'
    bg=turtle.Screen()
    ...
    for i in solution:
        ...
        drawasquare(bob,left_corner,right_corner)
...
def main():
    ...
    if (m*n)%(a*b)!=0:
        ...
        if list_answer==[]:
            ...
            list_answer_2=repeatergetaway(list_answer_1)
            list_answer_3=tuplize(list_answer_2)
            number=1
            for solution in list_answer_3:
                print(str(number)+': ',end='')
                print(solution)
                number+=1
            t=int(input('请选择一种进行可视化: '))
            turtles_see_you_again(list_answer_2[t-1],m,n,a,b)
